...
public class GunScript : MonoBehaviour
{
	...
	*/
    ...
    IEnumerator Reload_Animation()
    {
        if (bulletsIHave > 0 && bulletsInTheGun < amountOfBulletsPerLoad && !reloading /* && !aiming */)
        {
            ...
            {
                Debug.Log("'Reload Sound Source' missing.");
            }

            handsAnimator.SetBool("reloading", true);
            yield return new WaitForSeconds(0.30f); // Tiempo de espera simulando parte de la animación de recarga
            handsAnimator.SetBool("reloading", false);

            yield return new WaitForSeconds(reloadChangeBulletsTime - 0.30f); // Espera adicional para simular el resto de la animación

            if (!meeleAttack && pmS.maxSpeed != runningSpeed)
            {
                if (player.GetComponent<PlayerMovementScript>() && player.GetComponent<PlayerMovementScript>()._freakingZombiesSound)
                {
                ...
                int bulletsToLoad = (int)Mathf.Min(bulletsIHave, amountOfBulletsPerLoad - bulletsInTheGun);
                bulletsInTheGun += bulletsToLoad;
                bulletsIHave -= bulletsToLoad;

                Debug.Log("Reloaded: " + bulletsToLoad + " bullets into the gun. Remaining: " + bulletsIHave);
            }
            else
            {
                reloadSound_source.Stop();
                Debug.Log("Reload interrupted via melee attack or movement.");
            }

            reloading = false; // Marcar la recarga como completa
        }
        else if (bulletsIHave <= 0)
        {
            Debug.Log("Out of ammo entirely. Waiting 20 seconds before loading menu scene...");
            yield return new WaitForSeconds(10); // Espera 20 segundos antes de cargar la escena del menú
            SceneManager.LoadScene("menusalir"); // Carga la escena del menú
        }
    }
	...
	*/
    ...
    void Shooting()
    {
        ...
        waitTillNextFire -= Time.deltaTime * roundsPerSecond;
        CheckAmmo(); // Verificar el estado de la munición después de cada disparo
    }

    void OnAmmoDepleted()
    {
        Debug.Log("No more bullets in the gun!");
        if (bulletsIHave > 0)
        {
            StartCoroutine("Reload_Animation");
        }
        ...
        {
            Debug.Log("Out of ammo entirely!");
            StartCoroutine(Reload_Animation()); // Llama a la función de recarga cuando no hay municiones restantes
        }
    }
